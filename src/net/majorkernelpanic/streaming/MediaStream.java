...
public abstract class MediaStream implements Stream {
	...
	@Override
	public void setConsumer(RtpThread consumer, byte channelId) {
		mConsumer = consumer;
	...
	public synchronized void start() throws IllegalStateException, IOException {
		...
		if (mConsumer == null && mOutputStream == null && (mRtpPort <= 0 || mRtcpPort <= 0))
			throw new IllegalStateException("No tcp or udp comfirmed!!");
		if (mPacketizer != null) {
			mPacketizer.setDestination(mDestination, mRtpPort, mRtcpPort);
			mPacketizer.getRtpSocket().setOutputStream(mOutputStream, mChannelIdentifier);
			mPacketizer.setConsumer(mConsumer, mChannelIdentifier);
