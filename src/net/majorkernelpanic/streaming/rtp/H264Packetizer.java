...
public class H264Packetizer extends AbstractPacketizer implements Runnable {
	...
	public void run() {
		...
		try {
			socket.start();
			while (!Thread.interrupted() && socket.isAlive()) {
	...
	private void send() throws IOException, InterruptedException {
		...
		if (streamType == 0) {
			...
			if (naluLength > 100000 || naluLength < 0) {// error!try fix
				resync();
			}
		...
		} else if (type == 5 && sps != null && pps != null) {
			// We send two packets containing NALU type 7 (sps) and 8 (pps)
			// Those should allow the H264 stream to be decoded even if no SDP
			// was
			// sent to the decoder.
